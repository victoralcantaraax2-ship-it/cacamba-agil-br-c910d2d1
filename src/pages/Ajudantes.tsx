    ...
    try {
      ...
      const res = await fetch(`${supabaseUrl}/functions/v1/criar-pix`, {
        ...
        body: JSON.stringify({
          ...
          valor_custom: total,
          descricao_custom: `Serviço de Ajudante (${quantidade}x)`,
        }),
      });
      ...
      supabase.from("pix_leads" as any).insert({
        ...
        address: null,
        plan_id: null,
        plan_label: `Ajudante x${quantidade}`,
        amount: total,
        transaction_id: data.transaction_id || null,
        source: "ajudantes",
      }).then(() => {});
    } catch (err) {
      console.error("Erro ao gerar PIX ajudantes:", err);
      setStatus("idle");
    ...
    const poll = async () => {
      try {
        ...
        if (!cancelled && data.status === "paid") {
          setStatus("confirmed");
          toast({ title: "Pagamento confirmado!", description: "Serviço de ajudante confirmado." });
    ...
    <main className="min-h-screen bg-background flex flex-col items-center py-8 px-4">
      <div className="w-full max-w-md space-y-4">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <h1 className="text-lg font-extrabold text-foreground text-center">Serviço de Ajudante</h1>
          <p className="text-xs text-muted-foreground text-center">Mão de obra para carga e descarga</p>
        </div>
          ...
          <Card className="border-primary/30">
            <CardContent className="pt-6">
              <div className="flex flex-col items-center gap-1.5 mb-4">
                ...
                <span className="text-sm font-bold text-foreground">Serviço de Ajudante</span>
                <span className="text-[10px] text-muted-foreground">
                  {quantidade} ajudante{quantidade > 1 ? "s" : ""}
                </span>
              ...
              <div className="animate-in fade-in duration-500 space-y-3">
                ...
                {status === "confirmed" && (
                  <div className="flex items-center justify-center gap-2 py-3">
                    ...
                    <span className="text-sm font-bold text-accent">Pagamento confirmado! Redirecionando...</span>
                  </div>
                )}
              </div>
